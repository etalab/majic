...
function getCategorieLocal(record) {
  const categoriesLocalRevise = Object.values(record._pev)
    .filter(pev => pev.CCOCAC)
    .map(pev => modernizedCodes.CATEGORIES_LOCAL_REVISE[pev.CCOCAC])

  if (categoriesLocalRevise.length > 0) {
    return categoriesLocalRevise[0]
  }

  return modernizedCodes.CATEGORIES_LOCAL[record.CCONLC]
}
...
function createLocal(record) {
  return {
    ...
    categorieLocal: getCategorieLocal(record),
